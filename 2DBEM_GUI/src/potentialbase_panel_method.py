...
def BEM(inletvelocity, inletangle, geometry):
    ...
    for i in range(geometry.shape[0] - 1):
        for j in range(geometry.shape[0] - 1):
            if i == j:
                A_Matrix[i, j] = 0.5
                B1_Matrix[i, j] = strengh_of_source(
                    collocation[i, 0],
                    collocation[i, 1],
                    geometry[j, 0],
                    geometry[j, 1],
                ...
                C_Matrix[i, 0] = velocity[0] * math.sin(theta[i]) + velocity[
                    1
                ] * -math.cos(theta[i])
            else:
                var1 = strengh_of_doublet(
                    collocation[i, 0],
                    collocation[i, 1],
                    geometry[j, 0],
                    geometry[j, 1],
    ...
    for i in range(geometry.shape[0] - 1):
        wake_coefficient = strengh_of_doublet(
            collocation[i, 0],
            collocation[i, 1],
            geometry[0, 0],
            geometry[0, 1],
    ...
    for i in range(geometry.shape[0] - 1):
        PHI[i] = (
            collocation[i, 0] * math.cos(inletangle * math.pi / 180)
            + collocation[i, 1] * math.sin(inletangle * math.pi / 180)
            + Ans[i][0]
        )
    VelocityT[0] = 0
    VelocityT[-1] = 0
...
def strengh_of_source(xi, yi, xj1, yj1, xj2, yj2, index):
    ...
    if index == 1:
        a1 = 0.07957747155 * (
            XPt[0] * math.log(R1)
            - (XPt[0] - X2t[0]) * math.log(R2)
            + 2 * XPt[1] * (angle1 - angle2)
        )
    else:
